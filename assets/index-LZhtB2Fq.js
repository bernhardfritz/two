...
const S = 8192;
function X(e, t) {
  const o = F(e, e.VERTEX_SHADER, D), a = F(e, e.FRAGMENT_SHADER, P), n = Y(e, o, a), i = e.getAttribLocation(n, "a_position"), r = e.getAttribLocation(n, "a_model_matrix"), l = e.getAttribLocation(n, "a_texture_matrix"), m = e.getAttribLocation(n, "a_tint_color"), R = e.getUniformLocation(n, "u_projection"), d = e.getUniformLocation(n, "u_texture"), b = e.createVertexArray();
  e.bindVertexArray(b);
  ...
  e.bindBuffer(e.ARRAY_BUFFER, T), e.bufferData(e.ARRAY_BUFFER, S * E(t), e.DYNAMIC_DRAW);
  for (let s = 0; s < 4; s++) {
    const u = r + s;
    e.enableVertexAttribArray(u), e.vertexAttribPointer(u, 4, e.FLOAT, !1, E(t), L(t, "a_model_matrix") + s * U.vec4), e.vertexAttribDivisor(u, 1);
  }
  for (let s = 0; s < 4; s++) {
    const u = l + s;
    e.enableVertexAttribArray(u), e.vertexAttribPointer(u, 4, e.FLOAT, !1, E(t), L(t, "a_texture_matrix") + s * U.vec4), e.vertexAttribDivisor(u, 1);
  }
  return e.enableVertexAttribArray(m), e.vertexAttribPointer(m, 4, e.FLOAT, !1, E(t), L(t, "a_tint_color")), e.vertexAttribDivisor(m, 1), e.bindVertexArray(null), e.deleteBuffer(_), e.enable(e.BLEND), e.blendFunc(e.SRC_ALPHA, e.ONE_MINUS_SRC_ALPHA), (s) => {
    ...
    e.uniformMatrix4fv(
      ...
      !1,
      u
    ), e.uniform1i(d, 0), e.bindVertexArray(b), e.bindBuffer(e.ARRAY_BUFFER, T);
    const v = s.byteLength / E(t);
    for (let y = 0; y < v; y += S) {
      const B = Math.min(v - y, S);
      e.bufferSubData(e.ARRAY_BUFFER, 0, s, y * E(t) / Float32Array.BYTES_PER_ELEMENT, B * E(t) / Float32Array.BYTES_PER_ELEMENT);
      const I = e.TRIANGLE_STRIP;
      e.drawArraysInstanced(I, 0, 4, B);
    }
  };
}
...
async function H(e) {
  ...
  let a;
  const n = {};
  let i = [createImageBitmap(new ImageData(new Uint8ClampedArray([255, 255, 255, 255]), 1, 1))], r = 1, l = 1;
  o.importObject.env = {
    writeFile: function(m, R, d, b) {
      const _ = new Uint8Array(a.exports.memory.buffer), x = new TextDecoder().decode(_.subarray(m, m + R)), c = _.subarray(d, d + b);
      n[x] = c;
    },
    loadTexture: function(m, R, d, b) {
      const _ = new Uint8Array(a.exports.memory.buffer), x = new TextDecoder().decode(_.subarray(m, m + R)), c = n[x], h = new DataView(c.buffer, c.byteOffset, c.byteLength), A = h.getUint32(16), p = h.getUint32(20), w = i.length - 1, T = new Blob([c], { type: "image/png" }), s = createImageBitmap(T);
      i.push(s), r = Math.max(r, A), l = Math.max(l, p);
      const u = _.subarray(d, d + b), v = new DataView(u.buffer, u.byteOffset, u.byteLength);
      v.setUint32(0, w, !0), v.setUint32(4, A, !0), v.setUint32(8, p, !0);
    }
    // ... other functions
  }, C(fetch(new URL(\`\${e.baseURI}main.wasm\`)), o.importObject).then(async function(m) {
    a = m.instance, o.run(a);
    const R = t.createTexture();
    t.activeTexture(t.TEXTURE0), t.bindTexture(t.TEXTURE_2D_ARRAY, R), t.texParameteri(t.TEXTURE_2D_ARRAY, t.TEXTURE_MIN_FILTER, t.NEAREST), t.texParameteri(t.TEXTURE_2D_ARRAY, t.TEXTURE_MAG_FILTER, t.NEAREST), t.texImage3D(t.TEXTURE_2D_ARRAY, 0, t.RGBA8, r, l, i.length, 0, t.RGBA, t.UNSIGNED_BYTE, null), i = i.map((c, h) => c.then((A) => (t.texSubImage3D(t.TEXTURE_2D_ARRAY, 0, 0, 0, h, A.width, A.height, 1, t.RGBA, t.UNSIGNED_BYTE, A), A.close(), A))), await Promise.all(i), t.generateMipmap(t.TEXTURE_2D_ARRAY);
    ...
    let _ = 0;
    const x = (c) => {
      const h = a.exports.update(c - _, t.canvas.clientWidth, t.canvas.clientHeight, e.mouseX, e.mouseY, e.mouseButtons), A = Number(h >> 32n), p = Number(h & 0xffffffffn), w = new Uint8Array(a.exports.memory.buffer, A, p), T = new Float32Array(w.buffer, w.byteOffset, E(d) / 4 * p);
      b(T), _ = c, requestAnimationFrame(x);
    };
